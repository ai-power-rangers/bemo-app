...
class PieceTransformEngine {
    ...
    private func calculateRotationTransform(
        ...
        // Calculate valid rotation angles (rotation step increments that don't cause overlaps)
    ...
    private func calculateValidRotationAngles(
        ...
        // Generate angles based on rotation step
        let step = TangramConstants.rotationStepDegrees
        let allAngles = stride(from: -180.0, through: 180.0, by: step).map { $0 }
