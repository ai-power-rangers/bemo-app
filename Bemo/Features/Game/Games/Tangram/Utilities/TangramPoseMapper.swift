//
//  TangramPoseMapper.swift
//  Bemo
//
//  Centralized utility for coordinate space transformations between raw (DB/CV) and SpriteKit spaces
//

// WHAT: Maps between raw puzzle/CV space and SpriteKit rendering space with consistent conventions
// ARCHITECTURE: Shared utility providing single source of truth for all coordinate conversions
...
public struct TangramPoseMapper {
    
    // MARK: - Piece Geometry
    
    /// Calculate local centroid for any piece type (scaled to visual size)
    /// This is the reference point for all rotations and translations
    ...
    /// Convert raw centroid to SpriteKit position (convenience method)
    /// Same as spriteKitPosition but clearer intent for centroid usage
    public static func toSpriteKit(centroidRaw: CGPoint) -> CGPoint {
        return spriteKitPosition(fromRawPosition: centroidRaw)
    }
    
    /// Convert raw angle to SpriteKit rotation (convenience method)
    /// Same as spriteKitAngle but clearer intent
    public static func toSpriteKit(angleRaw: CGFloat) -> CGFloat {
        return spriteKitAngle(fromRawAngle: angleRaw)
    }
