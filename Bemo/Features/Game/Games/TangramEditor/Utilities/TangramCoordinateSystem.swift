...
class TangramCoordinateSystem {
    ...
    private static func calculateSinglePointAlignment(
            ...
            if let canvasPiece = existingPieces.first(where: { $0.id == connection.canvas.pieceId }) {
                ...
                if canvasEdgeIndex < canvasEdges.count {
                    ...
                    if pieceEdgeIndex < pieceEdges.count {
                        ...
                        print("DEBUG: Auto-rotating edge-to-edge connection")
                        print("DEBUG: Canvas edge angle: \(canvasEdgeAngle * 180 / .pi)°")
                        print("DEBUG: Piece edge angle: \(pieceEdgeAngle * 180 / .pi)°")
                        print("DEBUG: Final rotation: \(finalRotation * 180 / .pi)°")
                    }
                }
            }
        }
        ...
        if !isValidTransform(transform) {
            print("WARNING: Invalid transform in single point alignment")
            return nil
        }
    ...
    private static func calculateMultiPointAlignment(
        ...
        print("DEBUG ALIGN: Piece type: \(pieceType)")
        print("DEBUG ALIGN: Connection 1 - Type: \(connections[0].piece.type), Local: \(local1)")
        print("DEBUG ALIGN: Connection 2 - Type: \(connections[1].piece.type), Local: \(local2)")
        
        // Get canvas positions
        let canvas1 = connections[0].canvas.position
        let canvas2 = connections[1].canvas.position
        
        print("DEBUG ALIGN: Canvas 1: \(canvas1)")
        print("DEBUG ALIGN: Canvas 2: \(canvas2)")
        ...
        print("DEBUG ALIGN: Canvas vector: \(canvasVector)")
        print("DEBUG ALIGN: Local vector: \(localVector)")
        ...
        print("DEBUG ALIGN: Canvas angle: \(canvasAngle * 180 / .pi)°")
        print("DEBUG ALIGN: Local angle: \(localAngle * 180 / .pi)°")
        print("DEBUG ALIGN: Base rotation: \(baseRotation * 180 / .pi)°")
        print("DEBUG ALIGN: Rotation needed: \(rotationNeeded * 180 / .pi)°")
        ...
        print("DEBUG ALIGN: Rotated local1: \(rotatedLocal1)")
        
        transform.tx = canvas1.x - rotatedLocal1.x
        transform.ty = canvas1.y - rotatedLocal1.y
        
        print("DEBUG ALIGN: Translation: tx=\(transform.tx), ty=\(transform.ty)")
        ...
        print("DEBUG ALIGN: Rotated local2: \(rotatedLocal2)")
        print("DEBUG ALIGN: Canvas2 target: \(canvas2)")
        print("DEBUG ALIGN: Secondary error: \(secondaryError)")
        ...
        if secondaryError > tolerance {
            print("WARNING: Secondary point error: \(secondaryError) > tolerance: \(tolerance)")
            ...
            if connections.count == 2 {
                ...
                print("INFO: Connection 1 - canvas: \(conn1Type.canvas), piece: \(conn1Type.piece)")
                print("INFO: Connection 2 - canvas: \(conn2Type.canvas), piece: \(conn2Type.piece)")
                ...
                if hasEdgeConnection {
                    ...
                    print("INFO: Edge connection detected - different lengths are valid for sliding")
                    print("INFO: Accepting placement with edge length difference of \(secondaryError)")
                    return transform
                }
            }
            ...
            if bothVertices && secondaryError > tolerance {
                print("ERROR: Cannot satisfy both vertex connections - error too large")
                return nil
            }
            
            print("ERROR: Cannot place piece - connections cannot be satisfied")
            return nil
        }
        
        // Validate transform
        if !isValidTransform(transform) {
            print("WARNING: Invalid transform in multi-point alignment")
            return nil
        }
        
        print("SUCCESS: Multi-point alignment achieved with error: \(secondaryError)")
        return transform
    }
    ...
    private static func determineAlignmentTolerance(
        ...
        print("DEBUG ALIGN: Connection types - hasVertex: \(hasVertex), hasEdge: \(hasEdge)")
        
        // For vertex+edge combination, we need more tolerance
        // The edge might not be exactly the same length
        if hasVertex && hasEdge {
            print("DEBUG ALIGN: Using vertex+edge tolerance: 3.0")
            return 3.0 // More tolerance for mixed connections
        }
        
        // Both vertices - need tight alignment
        if connection1.piece.type.isVertex && connection2.piece.type.isVertex {
            print("DEBUG ALIGN: Using vertex-to-vertex tolerance: 1.5")
            return 1.5 // Slightly relaxed from 1.0
        }
        
        // Both edges - can have more tolerance (for sliding)
        if connection1.piece.type.isEdge && connection2.piece.type.isEdge {
            print("DEBUG ALIGN: Using edge-to-edge tolerance: 5.0")
            return 5.0 // More lenient for edge-to-edge
        }
        
        // Default
        print("DEBUG ALIGN: Using default tolerance: 2.0")
        return 2.0
    }
    ...
    static func debugPrintTransform(_ transform: CGAffineTransform, label: String = "Transform") {
        #if DEBUG
        print("\(label): [a:\(transform.a), b:\(transform.b), c:\(transform.c), d:\(transform.d), tx:\(transform.tx), ty:\(transform.ty)]")
        #endif
    }
}